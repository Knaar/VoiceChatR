...
public:
	...
	*/
	UFUNCTION(BlueprintCallable, Category = "RealTimeImport|Image")
		static URealTimeImportBPLibrary* LoadImageFileAsync(ERTIDirectoryType directoryType, FString filePath, FString eventID, TextureCompressionSettings compressionSettings = TextureCompressionSettings::TC_Default, bool SRGB = true, bool createMipMaps = false);
	...
	*/
	UFUNCTION(BlueprintCallable, Category = "RealTimeImport|Image")
		static void LoadImageBytes(FString name, TArray<uint8> bytes, UTexture2D*& texture, int32& errorCode, FString& errorMessage, TextureCompressionSettings compressionSettings = TextureCompressionSettings::TC_Default, bool SRGB = true, bool createMipMaps = false);
	...
	*/
	UFUNCTION(BlueprintCallable, Category = "RealTimeImport|Image")
		static URealTimeImportBPLibrary* LoadImageBytesAsync(FString name, TArray<uint8> bytes, FString eventID, TextureCompressionSettings compressionSettings = TextureCompressionSettings::TC_Default, bool SRGB = true, bool createMipMaps = false);
	...
	*/
	UFUNCTION(BlueprintCallable, Category = "RealTimeImport|Image")
		static void removeTextureFromCacheAndDestroyTexture(UTexture2D* texture, bool emptyTextureBeforeDeleting);
	...
	*/
	...
	/**
	* ONLY UE4! Creates an image file from the Texture2D  and returns the bytes. The bytes can be saved as a file or sent with a different plugin.
	*@param texture2D
	*@param type PNG,JPG and BMP are possible
	*@param quality In percent. 100 = highest quality. 1 = lowest quality. BMP is not compressed. With PNG, UE4 doesn't seem to change much. The biggest difference can be seen with JPG.
	*/
	...
	*/
	...
	UFUNCTION(BlueprintCallable, Category = "RealTimeImport|Mesh")
		static void breakModelStruct(FRTIModelStruct modelStruct, FString& name, FTransform& relativeTransform, TArray<FRTIMeshStruct>& meshStructs);
