...
void URealTimeImportMeshFBX::LoadFBXFile(FString dir, bool& success, FString& successMessage,
	TArray<FRTIModelStruct>& modelStructs, ERTICoordinateSystem coordinateSystem, bool autoDetectionNormalMap, bool useSRGB, bool createMipMaps) {
	...
	do {
		dataPosition = parseFbxBinaryData(success, successMessage, fbxFileData, dataPosition, fbxConnections, fbxVersion, coordinateSystem, dir);
	...
	for (auto& element : fbxConnections.model) {
		...
		Splitting them per mesh leads to much faster creation of meshes in Unreal.
	...
	UE_LOG(LogTemp, Display, TEXT("RealTimeImportPlugin: Vertices: %i UV0: %i Normals: %i VertexColors: %i "), verticesCount, UV0Count, normalsCount, vertexColorsCount);

	success = true;
	successMessage = "Vertices: " + FString::FromInt(verticesCount) + " UV0: " + FString::FromInt(UV0Count) + " Normals: " + FString::FromInt(normalsCount) + " VertexColors: " + FString::FromInt(vertexColorsCount);
							...
							if (!modelStruct->modelRotationXisSet) {
								...
								if (!modelStruct->modelRotationYisSet) {
									...
									if (!modelStruct->modelRotationZisSet) {
										...
										if (coordinateSystem == ERTICoordinateSystem::E_RightHanded) {
											modelStruct->relativeTransform.SetRotation(UKismetMathLibrary::Conv_VectorToQuaternion(FVector(modelStruct->relativeTransform.GetRotation().X, URealTimeImportMesh::parseBytesToDouble(fbxFileData, dataPosition), modelStruct->relativeTransform.GetRotation().Y)));
										}
										else {
											modelStruct->relativeTransform.SetRotation(UKismetMathLibrary::Conv_VectorToQuaternion(FVector(modelStruct->relativeTransform.GetRotation().X, modelStruct->relativeTransform.GetRotation().Y, URealTimeImportMesh::parseBytesToDouble(fbxFileData, dataPosition))));
										}
									}
								}
							}
